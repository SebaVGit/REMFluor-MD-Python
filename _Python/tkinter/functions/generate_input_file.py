...
def build_inp_data(state) -> dict:
    ...
    # v107: read the low-k foc from retardation_inputs.txt ("Low-K Zone"
    # foc) instead of hard-coding 0.001.  Previously the low-k foc the user
    # entered in §4 (e.g. 0.0035) was ignored and input.inp always wrote
    # 0.001 — mismatching the txt file.  Falls back to 0.001 if absent.
    focm  = (retard_data.get('Low-K') or retard_data.get('Low-k')
             or {}).get('foc', 0.001)
    ...
    # v106: scale the Darcy velocity by the transmissive-zone volume
    # fraction before writing it to input.inp.  Only the transmissive
    # fraction of the cross-section carries advective flow, so the
    # effective Darcy velocity the solver uses is vd * volfrac.  This must
    # come AFTER any calibration override of volfrac above.  When
    # volfrac = 1.0 (no low-k zone / no heterogeneity_inputs.txt) it's a
    # no-op, so existing fully-transmissive models are unaffected.
    if vd is not None:
        try:
            vd = vd * float(volfrac)
        except (TypeError, ValueError):
