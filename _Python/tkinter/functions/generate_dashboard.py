...
# Helper function to convert Excel cell notation (e.g., "A8") to openpyxl cell access
def get_cell_value(ws, cell_ref):
    """Get cell value from openpyxl worksheet using Excel notation (e.g., 'A8')"""
...
# Helper function to convert Excel range notation (e.g., "A8:A18") to list of values
def get_range_values(ws, range_ref):
    """Get range values from openpyxl worksheet using Excel notation (e.g., 'A8:A18')"""
    # Parse range like "A8:A18" or "V34:V41"
...
def read_model_data_sheet(excel_app, calibration_file_path):
    """Read Model Data sheet from calibration Excel file and return DataFrame"""
    original_visible = None
    try:
        # Temporarily make Excel invisible to prevent calibration file from appearing
        original_visible = excel_app.Visible
        excel_app.Visible = False
        
        wb_cal = excel_app.Workbooks.Open(calibration_file_path)
        try:
            ws_model = wb_cal.Sheets('Model Data')
        except:
            try:
                ws_model = wb_cal.Sheets('ModelData')
            except:
                ws_model = None
        
        if ws_model is None:
            wb_cal.Close(SaveChanges=False)
            # Restore Excel visibility
            if original_visible is not None:
                excel_app.Visible = original_visible
            return None
        
        # Read data from Excel sheet into DataFrame
        # Find the header row
        header_row = None
        for row in range(1, 20):
            # Check for common header patterns
            cell_a = ws_model.Range(f"A{row}").Value
            cell_b = ws_model.Range(f"C{row}").Value
            if cell_a is not None and cell_b is not None:
                cell_a_str = str(cell_a).strip().lower()
                cell_b_str = str(cell_b).strip().lower()
                if ('Date' in cell_a_str or 'time' in cell_a_str) and ('Analyte' in cell_b_str or 'compound' in cell_b_str):
                    header_row = row
                    break
        ...
        # Read data starting from header_row
        data_rows = []
        max_row = ws_model.UsedRange.Rows.Count if ws_model.UsedRange else 1000
        
        for row in range(header_row + 1, min(header_row + 1000, max_row + 1)):
            date_val = ws_model.Range(f"A{row}").Value
            analyte_val = ws_model.Range(f"C{row}").Value
            conc_val = ws_model.Range(f"D{row}").Value
            unit_val = ws_model.Range(f"E{row}").Value if ws_model.Range(f"D{row}").Value is not None else ""
            ...
            if date_val is not None and analyte_val is not None and conc_val is not None:
                # Try to get well/location name
                well_val = None
                for col in ['B']:
                    well_cell = ws_model.Range(f"{col}{row}").Value
                    if well_cell is not None:
                        well_val = str(well_cell).strip()
                        break
        ...
        # Restore Excel visibility even if there's an error
        try:
            excel_app.Visible = original_visible
        except:
            pass
...
def main(workbook_path=None, sheet_name=None):
    # Get workbook path from command-line argument if provided
    ...
    if workbook_path:
        input_excel = os.path.abspath(workbook_path)
    else:
        input_excel = os.path.abspath(r"C:\REMChlorMD_64bit\0_REMFluorMD_64\REMFluor-MD Interface Storyboard v2.6.xlsm")
        workbook_path = input_excel
    
    workbook_dir = os.path.dirname(os.path.abspath(input_excel))
    os.chdir(workbook_dir)
    
    # Try to save the Excel file if it's open in Excel (using xlwings)
    try:
        ...
        try:
            ...
            for book in app.books:
                if os.path.abspath(book.fullname) == os.path.abspath(input_excel):
                    wb_xw = book
                    break
            
            # If workbook found and open, save it
            if wb_xw is not None:
                wb_xw.save()
                print(f"Saved Excel workbook: {input_excel}")
        except Exception as e:
            # If xlwings fails, continue without saving
            print(f"Could not save Excel file via xlwings: {e}")
    except ImportError:
        # xlwings not available, continue without saving
        pass
    except Exception as e:
        # Any other error, continue without saving
    ...
    # Read initial values using openpyxl
    version_flag = get_cell_value(ws, "A8")  # 1 is simple, 2 is detailed
    unit_flag = get_cell_value(ws, "AD1")  # 1 is Feet, 2 is Meters
    if unit_flag == 1:
    ...
    # Initialize Excel COM object for calibration file reading (if needed)
    excel = None
    try:
    ...
    # Initialize calibration_file to None (will only be set for Detailed versions)
    calibration_file = None
    
    # Read MW_names from calibration Excel file (only for Detailed versions, not Simple)
    MW_names = []
    if version_flag != 1:  # Only read calibration_inputs.txt for Detailed versions
        calibration_file = read_calibration_inputs("calibration_inputs.txt")
        if calibration_file and os.path.exists(calibration_file) and excel:
            try:
                # Temporarily make Excel invisible to prevent calibration file from appearing
                original_visible = excel.Visible
                excel.Visible = False
                
                # Try to open the calibration Excel file
                wb_cal = excel.Workbooks.Open(calibration_file)
                try:
                    # Try "Model Location" sheet first, then "Location Name" if it exists
                    try:
                        ws_cal = wb_cal.Sheets('Model Location')
                    except:
                        ws_cal = wb_cal.Sheets(1)  # Use first sheet as fallback

                    # Find the "Location Name" column by searching header row
                    header_row = 1
                    location_col = None

                    # Search for "Location Name" in header row (check columns A-Z)
                    for col_idx in range(1, 27):  # A-Z
                        col_letter = chr(64 + col_idx)  # Convert 1-26 to A-Z
                        cell_value = ws_cal.Range(f"{col_letter}{header_row}").Value
                        if cell_value is not None:
                            cell_str = str(cell_value).strip().lower()
                            if 'location' in cell_str and 'name' in cell_str:
                                location_col = col_letter
                                break

                    # If "Location Name" column found, read values from that column
                    if location_col:
                        for row in range(header_row + 1, header_row + 20):  # Read up to 20 rows
                            cell_value = ws_cal.Range(f"{location_col}{row}").Value
                            if cell_value is not None and str(cell_value).strip():
                                MW_names.append(str(cell_value).strip())
                            elif MW_names:  # Stop if we hit an empty cell after finding some values
                                break
                    else:
                        # Fallback: try column A if no header found
                        for row in range(2, 20):
                            cell_value = ws_cal.Range(f"A{row}").Value
                            if cell_value is not None and str(cell_value).strip():
                                MW_names.append(str(cell_value).strip())
                            elif MW_names:
                                break
                finally:
                    # Close the workbook and restore Excel visibility
                    wb_cal.Close(SaveChanges=False)
                    excel.Visible = original_visible
            except Exception as e:
                print(f"Warning: Could not read MW_names from calibration file: {e}")
                MW_names = []
    ...
    StartY = get_cell_value(ws, "E18")
    Total_Depth = get_cell_value(ws, "E13")
    ...
    # Read PFAA1 and PFAA2 data from Excel cells V34-V41 and X34-X41
    # These correspond to monitoring wells (rows 34-41)
    excel_PFAA1_data = {}  # Dictionary: {well_name: concentration}
    excel_PFAA2_data = {}  # Dictionary: {well_name: concentration} - only used if ncomp == 2
    excel_data_year = None  # Year for Excel data from R36
    
    # Ensure MW_names has enough elements, pad with default names if needed
    if version_flag == 1:
        MW_names_raw = get_range_values(ws, "U34:U40")
        MW_names = [val for val in MW_names_raw] if MW_names_raw else []
        try:
            # Read the year for Excel data from cell R36
            excel_data_year_val = get_cell_value(ws, "R36")
            if excel_data_year_val is not None:
                try:
                    excel_data_year = float(excel_data_year_val)
                except (ValueError, TypeError):
                    pass
            # Read V34-V41 (PFAA1) and X34-X41 (PFAA2) - only read PFAA2 if ncomp == 2
            PFAA1_range = get_range_values(ws, "V34:V41")
            PFAA2_range = get_range_values(ws, "X34:X41") if ncomp == 2 else None
       
            # Excel ranges return 2D arrays (list of lists), convert to flat list
            # Handle both single cell and multi-cell ranges
            PFAA1_values = []
            if PFAA1_range:
                if isinstance(PFAA1_range, (list, tuple)):
                    for item in PFAA1_range:
                        if isinstance(item, (list, tuple)):
                            PFAA1_values.append(item[0] if len(item) > 0 else None)
                        else:
                            PFAA1_values.append(item)
                else:
                    PFAA1_values = [PFAA1_range]
            
            PFAA2_values = []
            if ncomp == 2 and PFAA2_range:
                if isinstance(PFAA2_range, (list, tuple)):
                    for item in PFAA2_range:
                        if isinstance(item, (list, tuple)):
                            PFAA2_values.append(item[0] if len(item) > 0 else None)
                        else:
                            PFAA2_values.append(item)
                else:
                    PFAA2_values = [PFAA2_range]
            
            # Match with MW_names (assuming rows 34-41 correspond to wells)
            for idx in range(min(len(MW_names), 8)):  # Up to 8 wells (rows 34-41)
                if idx < len(PFAA1_values) and PFAA1_values[idx] is not None:
                    try:
                        PFAA1_val = float(PFAA1_values[idx])
                        if PFAA1_val > 0:  # Only store valid positive values
                            excel_PFAA1_data[MW_names[idx]] = PFAA1_val
                    except (ValueError, TypeError):
                        pass
                
                # Only process PFAA2 if ncomp == 2
                if ncomp == 2 and idx < len(PFAA2_values) and PFAA2_values[idx] is not None:
                    try:
                        PFAA2_val = float(PFAA2_values[idx])
                        if PFAA2_val > 0:  # Only store valid positive values
                            excel_PFAA2_data[MW_names[idx]] = PFAA2_val
                    except (ValueError, TypeError):
                        pass
        except Exception as e:
            print(f"Warning: Could not read PFAA1/PFAA2 data from Excel: {e}")
    ...
    for idx, fname in enumerate(obs_well_files):
        if idx == 0:
            ...
            df['well_file'] = MW_names[idx]
            dfs.append(df)
    df_obs_MW = pd.concat(dfs, ignore_index=True) # monitoring wells
    ...
    # Calculate Source Mass from the spreadsheet
    Source_Width = get_cell_value(ws, "E15")
    Source_Thick = get_cell_value(ws, "E16")
    Velocity = get_cell_value(ws, "E22")
    EndYear = get_cell_value(ws, "E19")
...
<html>
                ...
                html.Div([
                    ...
                    html.Div([
                        dcc.Dropdown(
                            ...
                            value=["C2well"] if (version_flag == 1 or not precursor_flag) else ["C1well"],
    ...
    def update_concentration_dropdown_options(analysis_type):
        ...
        if analysis_type == 5 and version_flag == 1:
            ...
            # For other cases, show all options (excluding C4well if ncomp == 1)
            preferred_value = "C2well" if (version_flag == 1 or not precursor_flag) else "C1well"
    ...
    def update_plot(time_val, x_val, y_val, z_val, analysis_type, well_vals, conc_vals, x_massd_val, log_scale):
        ...
        if analysis_type == 1:
            ...
            if well_vals and conc_vals:
                ...
                # Plot Observation Point for Simple Version
                # Get time for plotting Excel data points from cell R36
                    ...
                    for well in well_vals:
                        for conc in conc_vals:
                            ...
                            if obs_analyte_name and obs_value is not None:
                                ...
                                color_idx += 1
                # Plot Observation Points for Detailed Version (one trace per well-analyte combination)
                if version_flag == 2:
                    selected_analytes = {
