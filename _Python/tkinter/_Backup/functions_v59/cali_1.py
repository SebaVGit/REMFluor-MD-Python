...
def run(app, parent=None) -> bool:
    ...
    root.title("Calibration in progress")
    root.configure(bg="#F0F0F0")
    try:
        root.transient(parent or app)
    except Exception:
        pass
    label = tk.Label(root, text="Starting...", font=("Arial", 16, "bold"),
                     bg="#F0F0F0", padx=20, pady=14, justify="center")
    label.pack(padx=24, pady=(20, 6))
    detail = tk.Label(root, text="", font=("Arial", 11), bg="#F0F0F0",
                      fg="#444", justify="center")
    detail.pack(padx=24, pady=(0, 18))

    stop_flag = {"stop": False}
    btn_row = tk.Frame(root, bg="#F0F0F0"); btn_row.pack(pady=(0, 16))
    def _cancel():
        stop_flag["stop"] = True
        label.config(text="Cancelling...")
    tk.Button(btn_row, text="Cancel", width=12, font=("Arial", 11),
              command=_cancel).pack(side="left", padx=10)

    root.update_idletasks()
    w = max(root.winfo_reqwidth() + 48, 540)
    h = max(root.winfo_reqheight() + 32, 220)
    try:
    ...
    def _on_done():
        ...
        if res is None:
            ...
            return
        # Push best params back to UI (only the v_calib_mid cells)
        ...
        for child in btn_row.winfo_children():
            child.destroy()
        tk.Button(btn_row, text="Close", width=12, font=("Arial", 11),
