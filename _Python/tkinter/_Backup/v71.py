...
def _load_optimal_model(app, path):
    """Read a previously-saved optimal_model.txt and push the lo/mid/hi
    triplets back into the calibration widgets.  Format must match
    what's written in the Save_Optimal_Model branch above:
        use=False lo=... mid=... hi=...
    ...
    with open(path, "r", encoding="utf-8") as fp:
        for line in fp:
            ...
            if not m:
                continue
            if n >= min(len(chk), len(lo), len(mid), len(hi)):
                break
            use_s, lo_s, mid_s, hi_s = m.groups()
            try:
                chk[n].set(use_s.lower() == "true")
            except Exception:
                pass
            lo[n].set(lo_s)
            mid[n].set(mid_s)
...
def run_script(macro_name, extra_args=None):
    ...
    if macro_name == "Load_Optimal_Data":
        ...
        # Model) and push the saved ranges back into the §calibration
        # widgets.  If the file doesn't exist, tell the user.
        if _app_ref is None:
            return
        ...
        try:
            ...
            # Propagate the loaded optimal Mid values into the §1-§7
            # source cells so Run Optimal Model uses the new optimum
            # (not the user's pre-calibration inputs).
            ...
            messagebox.showinfo("Load Optimal Data",
                                ...
                                f"{path}\n\n"
                                "Mid values propagated to §1-§7 inputs;\n"
...
class REMFluorApp(tk.Tk):
    ...
    def _build_vars(self):
        ...
        def _recompute_run_time(*_):
            try:
                ...
                # Default cell sizes — match the cellsize_input.txt
    ...
    def _build_calibration_panel(self, parent):
        ...
        def _src_K():
            """Hydraulic Conductivity in m/year.

            Resolution order:
              1. If the user used the §3 GW Velocity Calculator (and
                 thus gwvelocity_inputs.txt exists), use the user's
                 typed K — unit-converted from the chosen unit
                 (m/day, ft/day, etc.) to m/yr.  This is the actual
                 K, NOT the Darcy velocity.
              2. Otherwise, fall back to v_darcy (§3 Darcy velocity
                 cell).  Per user's rule: "if there is no more
                 information we can use same as the velocity, as
                 long as there is a value in that velocity cell" —
                 i.e. assume gradient=1 so K_eff equals the velocity.
              3. If neither exists, blank.
            """
            d = _read_gw_inputs()
            if "k_value" in d:
                ...
                return f"{k_myr:g}"
            try:
                return str(self.v_darcy.get()).strip()
            except Exception:
                return ""
        ...
        def _src_i():
            ...
            Resolution:
              1. If GW Calc was used, return user's typed gradient.
              2. Else 1 — a sensible default that makes K_eff
                 equal v_darcy (matches the K fallback above)."""
            d = _read_gw_inputs()
            if "gradient" in d:
        ...
        REVERSE_MAP = {
            "Source Start Year (nt)":                  self.v_yr_start,
            "Hydraulic Conductivity (k)":              self.v_darcy,  # K_eff = vd
            "Effective Porosity (porf)":               self.v_porf,
            "Retardation Factor of PFAA-1 (ock(2))":   self.v_ret_trans1,
            "Retardation Factor of PFAA-2 (ock(4))":   self.v_ret_trans2,
            "Longitudinal Dispersivity (alphax (m))":  self.v_alpha_l,
        ...
        def _push_calib_mids_to_inputs():
            ...
            value back to the corresponding source StringVar.  Used
            after a calibration run finishes (best params → app)
            and after Load Optimal Data."""
