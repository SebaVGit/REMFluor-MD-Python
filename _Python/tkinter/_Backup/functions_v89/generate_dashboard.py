...
def main(workbook_path=None, sheet_name=None):
    ...
    try:
        sy_addr = "R36" if version_flag == 1 else "Y74"
        sy_val = get_cell_value(ws, sy_addr)
        if sy_val is None:
            sy_val = get_cell_value(ws, "R36") or get_cell_value(ws, "Y74")
    ...
    # v89: pre-compute an overall RMSE summary from §10 observed
    # concentrations vs. simulated at the sample year, so it can be
    # displayed in the dashboard header (under the title) instead of
    # only appearing inside the Observation Wells analysis tab.
    rmse_header_lines = []
    try:
        if (excel_PFAA1_data or excel_PFAA2_data) and excel_data_year is not None:
            sim_times_avail = (df_obs_MW['Time yr'] + StartY).unique() \
                if 'Time yr' in df_obs_MW.columns and len(df_obs_MW) > 0 else np.array([])
            tgt_year = find_closest_date(excel_data_year, sim_times_avail) \
                if len(sim_times_avail) > 0 else None
            for kind, obs_dict, sim_col, default_label in [
                ("PFAA-1", excel_PFAA1_data, "C2well",
                 PFAS_names[1] if len(PFAS_names) > 1 else "PFAA-1"),
                ("PFAA-2", excel_PFAA2_data, "C4well",
                 PFAS_names[3] if len(PFAS_names) > 3 else "PFAA-2"),
            ]:
                if not obs_dict or tgt_year is None:
                    continue
                obs_pts, pred_pts = [], []
                for well, obs_v in obs_dict.items():
                    if obs_v is None or obs_v <= 0:
                        continue
                    well_data = df_obs_MW[
                        (df_obs_MW['well_file'] == well)
                        & (np.abs((df_obs_MW['Time yr'] + StartY) - tgt_year) < 0.01)
                    ]
                    if len(well_data) > 0 and sim_col in well_data.columns:
                        pred = float(well_data[sim_col].mean())
                        if pred > 0:
                            obs_pts.append(obs_v)
                            pred_pts.append(pred)
                if obs_pts:
                    val = calculate_rmsle(np.array(obs_pts), np.array(pred_pts))
                    if val is not None:
                        rmse_header_lines.append(
                            f"{default_label}: RMSLE = {val:.3g} (n={len(obs_pts)})"
                        )
    except Exception as e:
        print(f"[dashboard] RMSE header calc failed: {e}")
    rmse_header_text = (" • ".join(rmse_header_lines)
                        if rmse_header_lines
                        else "RMSLE: no observed/simulated overlap at sample year")
    ...
    app.layout = html.Div([
        html.Div([
            html.Div([
                ...
                # v89: RMSLE summary directly under the title.
                html.Div(rmse_header_text,
                         className="dashboard-rmse",
                         style={
                             "textAlign": "center",
                             "fontSize": "1.5rem",
                             "fontWeight": "600",
