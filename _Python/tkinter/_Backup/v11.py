...
class REMFluorApp(tk.Tk):
    ...
    def _build_s7_source(self, parent):
        ...
        dd_pre1 = dropdown(f, self.v_pfaa3, PRE_CHOICES, width=8,
                           bg=BG_PULLDOWN)
        dd_pre1.grid(row=1, column=6, padx=4)
        dd_pre2 = dropdown(f, self.v_pfaa4, PRE_CHOICES, width=8,
                           bg=BG_PULLDOWN)
        dd_pre2.grid(row=1, column=7, padx=4)
    ...
    # XLSM-authoritative layout (Detailed_2 sheet):
    #   LEFT half:
    #     Row 22 — Model PSB? + species headers (Unit | PFOS | None | Pre1 | Pre2)
    #     Row 23 — PSB's Freundlich "a"   (V/X/Z/AB values)
    #     Row 24 — PSB's Freundlich Kf    (V/X/Z/AB values)
    #     Row 26 — Converted Freundlich Kf (locked, V/X/Z/AB)
    #     Row 28 — Year PSB Installed    (entry)
    #     Row 29 — PSB Dist. from Source (entry)
    #     Row 30 — Where to Get Freundlich / Simple CAC Barrier buttons
    #   RIGHT half:
    #     Section9_1.png  (top)
    #     Total Width of PSB in X-Direction = 4 (m)
    #     PSB Loading 'fcac'                = 0.24 %
    #     # of cells in PSB in x direction  = 4 (-)
    #
    # Simple version: only the PFAA1 + PFAA2 columns.  Detailed adds
    # Precursor 1 + Precursor 2.  Right-side stack is identical in both.
    # ─────────────────────────────────────────────────────────────────────
    def _build_s9_psb(self, parent):
        ...
        # ═══ LEFT — Freundlich grid + Year/Dist + buttons ═══════════════
        ...
        h_pre1 = tk.Label(f, text="PFAA 1-able", font=FONT_LABEL_B,
                          bg=BG_MAIN)
        h_pre1.grid(row=0, column=5, padx=4)
        h_pre2 = tk.Label(f, text="None", font=FONT_LABEL_B,
                          bg=BG_MAIN)
        h_pre2.grid(row=0, column=6, padx=4)
        _mark_detailed(h_pre1, h_pre2)

        # Helper for one Freundlich row
        ...
        help_link(f, "OpenTable9_3").grid(row=4, column=7)

        # Row 5 — PSB Dist. from Source
        ...
        # Buttons row (left half, beneath Year/Dist)
        ...
        # ═══ RIGHT — Image (top) + Total Width / fcac / cells stack ═════
        scale = getattr(self, "_dpi_scale", 1.0)
        self._figures["s9_1"] = _load_figure("Section9_1.png",
                                             target_height=int(75 * scale))
        if self._figures["s9_1"] is not None:
            tk.Label(right, image=self._figures["s9_1"], bg=BG_MAIN, bd=0
                     ).pack(anchor="w", pady=(0, 6))

        rg = tk.Frame(right, bg=BG_MAIN)
        rg.pack(anchor="w")
        for i, (lbl, var, unit, helpm) in enumerate([
            ...
            make_entry(rg, var, width=6).grid(row=i, column=1, padx=4)
            tk.Label(rg, text=unit, font=FONT_LABEL_SM, bg=BG_MAIN
                     ).grid(row=i, column=2, sticky="w")
            if helpm:
                help_link(rg, helpm).grid(row=i, column=3, padx=2)
