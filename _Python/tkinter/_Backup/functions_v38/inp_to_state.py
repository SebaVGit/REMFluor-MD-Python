...
def write_inp_to_state(state, data: dict, additional: dict, unit_flag: int):
    ...
    for i, src in enumerate(data.get('source_zone_data', [])[:11]):
        ...
        state.set(f"V{row}", src['c2'])           # PFAA 1
        if data.get('ncomp', 1) == 2:
            state.set(f"X{row}", src['c4'])       # PFAA 2
        if data.get('ipre', 0) == 1:
            state.set(f"Z{row}", src['c1'])       # Precursor 1
            if data.get('ncomp', 1) == 2:
                state.set(f"AB{row}", src['c3'])  # Precursor 2
    ...
    if iwall:
        ...
        if psb_load is not None:
            # PSB Loading 'fcac' — UI cell AA82 (legacy AH28).
            state.set("AA82", psb_load)
        ...
        if psb_load:
            ...
            state.set("V24", (data.get('fcackf2') or 0) / psb_load)
            state.set("X24", (data.get('fcackf4') or 0) / psb_load)
            if data.get('ipre', 0) == 1:
                state.set("Z24",  (data.get('fcackf1') or 0) / psb_load)
                state.set("AB24", (data.get('fcackf3') or 0) / psb_load)
    else:
