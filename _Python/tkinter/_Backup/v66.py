...
class REMFluorApp(tk.Tk):
    ...
    def scroll_to_calibration(self):
        """Scroll the canvas so the §calibration quadrant lands at
        the top of the visible canvas.  Wired to §11 'Run Model with
        Auto-Calibration' — the user clicks it expecting to fill in
        Step 2-4 first, not to kick off the optimizer immediately.

        Computes the fraction from the calibration panel's actual
        widget y-coordinate inside the scrolled `inner` frame, so
        the scroll target stays correct regardless of layout
        reshuffles.  Falls back to a sensible hardcoded fraction
        only if the panel reference or geometry isn't available
        (very early in startup, etc.)."""
        try:
            self.update_idletasks()
            panel = getattr(self, "_calib_panel_frame", None)
            scroll_done = False
            if panel is not None and panel.winfo_exists():
                # winfo_y() returns y relative to the panel's parent.
                # Walk up until the parent IS self.inner (the
                # scrolled frame), summing y-offsets along the way.
                y = 0
                w = panel
                ...
                # Clamp to [0, 1] — yview_moveto rejects values
                # outside that range silently on some Tk builds.
                frac = max(0.0, min(1.0, y / total))
                self.canvas.yview_moveto(frac)
                self.canvas.xview_moveto(0.0)
                scroll_done = True
            if not scroll_done:
                # Fallback — calibration panel lives near the
                # bottom of the right column, so 0.55 typically
                # puts the header in view.
