...
def run_script(macro_name, extra_args=None):
    ...
    if macro_name == "Run_Machine_Based_Calibration":
        # Pure-Python stub for the calibration run.  The original Excel
        # macro shelled out to dist/cali_1.exe which loops over the
        # parameter grid.  In this standalone build we don't yet have
        # a Python optimizer; instead we:
        #   1. Save calibration inputs (input.inp + calibration_inputs.txt)
        #   2. Tell the user the run is queued and explain that the
        #      Python calibration optimizer is not yet implemented.
        # Once the backend lands, swap this to call the optimizer.
        if _FUNCS_LOADED and _app_ref is not None:
            try:
                ok = generate_input_file.run(_app_ref)
                if ok:
                ...
                messagebox.showinfo(
                    ...
                    "Calibration inputs saved to:\n"
                    f"{BASE_DIR}\n\n"
                    "The standalone Python calibration optimizer is not "
                    "wired up yet; the saved files are ready for it.\n\n"
                    "For now you can:\n"
                    "  • Click '5. Run Optimal Model' to run a single\n"
                    "    forward simulation with the current inputs.")
            except Exception as exc:
