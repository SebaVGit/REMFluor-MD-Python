    ...
    if _FUNCS_LOADED and _app_ref is not None:
        ...
        if macro_name == "RunPythonScript":
            ...
            if ok:
                ...
                inp_path = os.path.join(BASE_DIR, 'input.inp')
                out_path = os.path.join(BASE_DIR, 'output.out')
                exe_path = os.path.join(BASE_DIR, 'remfluor_v8a.exe')
                if not os.path.exists(exe_path):
                    messagebox.showerror("Error",
                        f"Model executable not found:\n{exe_path}")
                    return
                _launch_exe(exe_path, [], wait=False)
            return
...
class REMFluorApp(tk.Tk):
    ...
    def _build_s5_transport(self, parent):
        ...
        for i, (lbl, v1, v2, v3, v4, unit, helpm) in enumerate(ret_rows):
            ...
            make_entry(f, v1, width=8, bg=BG_LOCKED).grid(row=2+i, column=1, padx=4)
            make_entry(f, v2, width=8, bg=BG_LOCKED).grid(row=2+i, column=2, padx=4)
