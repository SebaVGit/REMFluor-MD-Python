...
class REMFluorApp(tk.Tk):
    ...
    def scroll_to_calibration(self):
        """Scroll the canvas so the §calibration quadrant is in view.
        Wired to §11 'Run Model with Auto-Calibration' — the user
        clicks it expecting to fill in Step 2-4 first, not to kick
        off the optimizer immediately.
        """
        try:
            self.update_idletasks()
            # Total scrollable height (set by inner.bbox).  yview_moveto
            # takes a fraction 0..1 of that; we jump near the bottom
            # since the calibration panel lives at the bottom of the
            # right column.  Use 0.62 to land at the calibration
            # header (above the buttons) — adjust if the layout
            # changes again.
            self.canvas.yview_moveto(0.62)
            self.canvas.xview_moveto(0.0)
        except Exception:
            pass
        try:
