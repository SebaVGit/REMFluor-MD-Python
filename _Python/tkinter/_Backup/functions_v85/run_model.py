...
def _check_dashboard_imports(py_exec: str):
    ...
    # v85: reverted to the original import set — the dashboard once
    # again reads .xlsm cell values via openpyxl + uses pywin32 for
    # the calibration .xlsx (Model Data sheet).  v82's JSON-only
    # decoupling caused crashes when the user's tkinter form was
    # blank, so we went back to the workbook-reading version that
