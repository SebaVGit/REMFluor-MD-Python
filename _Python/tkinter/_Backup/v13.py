...
class REMFluorApp(tk.Tk):
    ...
    def _build_s7_source(self, parent):
        ...
        for i in range(11):
            ...
            make_entry(f, self.v_src_years[i], width=8,
                       bg=BG_INPUT_BLUE, justify="right"
    ...
    #   TOP:    [Section9_1.png on left]   Year PSB Installed [entry] ?
    #                                      PSB Dist. From Source [entry] (m) ?
    #   MIDDLE: Model PSB? + Unit | PFOS | None | Pre1 | Pre2  (headers)
    #           Freundlich "a"   | -                 | sky blue cells
    #           Freundlich Kf    | (mg/kg) peach     | sky blue cells
    #           Conv. Freundlich | (ug/kg) BLACK     | sky blue cells
    #   RIGHT:  Total Width / PSB Loading 'fcac' / # of cells (stacked)
    #   BOTTOM: [Where to Get Freundlich Parameters] [Simple CAC ...]
    #
    # Simple = 2 conc cols (PFOS/None).  Detailed = 4 (+ Pre1, Pre2).
    # ─────────────────────────────────────────────────────────────────────
    def _build_s9_psb(self, parent):
        ...
        # ═══ TOP — Figure + Year PSB Installed + PSB Dist From Source ═══
        top_row = tk.Frame(parent, bg=BG_MAIN)
        ...
        if self._figures["s9_1"] is not None:
            tk.Label(top_row, image=self._figures["s9_1"], bg=BG_MAIN, bd=0
                     ).pack(side="left", anchor="nw")

        # To the right of the image: Year PSB Installed + PSB Dist From Source
        top_right = tk.Frame(top_row, bg=BG_MAIN)
        top_right.pack(side="left", anchor="nw", padx=(20, 0))

        yr_row = tk.Frame(top_right, bg=BG_MAIN); yr_row.pack(anchor="w")
        tk.Label(yr_row, text="Year PSB Installed", font=FONT_LABEL,
                 bg=BG_MAIN).pack(side="left")
        make_entry(yr_row, self.v_psb_yr, width=8
                   ).pack(side="left", padx=4)
        help_link(yr_row, "OpenTable9_3").pack(side="left", padx=(2, 0))

        dist_row = tk.Frame(top_right, bg=BG_MAIN)
        dist_row.pack(anchor="w", pady=(4, 0))
        tk.Label(dist_row, text="PSB Dist. From Source", font=FONT_LABEL,
                 bg=BG_MAIN).pack(side="left")
        make_entry(dist_row, self.v_psb_dist, width=8
                   ).pack(side="left", padx=4)
        tk.Label(dist_row, text="(m)", font=FONT_LABEL_SM, bg=BG_MAIN,
                 fg="#C00000").pack(side="left")

        # ═══ MIDDLE+RIGHT — Freundlich grid (left) + Total Width stack (right)
        mid_outer = tk.Frame(parent, bg=BG_MAIN)
        mid_outer.pack(fill="x", pady=(0, 4))

        left = tk.Frame(mid_outer, bg=BG_MAIN)
        left.pack(side="left", anchor="nw")
        right = tk.Frame(mid_outer, bg=BG_MAIN)
        right.pack(side="left", anchor="nw", padx=(30, 0))
        ...
        # Helper for one Freundlich row.  unit_bg controls the unit-cell
        # background — peach for editable rows, BLACK for the Converted
        # row (per user feedback).  Value cells are always sky blue.
        def _freundlich_row(r, label, unit, v1, v2, v3, v4, helpm,
                            unit_bg=BG_PULLDOWN, unit_fg=FG_INPUT):
            tk.Label(f, text=label, font=FONT_LABEL, bg=BG_MAIN
                     ).grid(row=r, column=0, sticky="w", pady=1)
            tk.Label(f, text=unit, font=FONT_LABEL_SM, bg=unit_bg,
                     fg=unit_fg, relief="solid", bd=1, padx=2
                     ).grid(row=r, column=2, padx=4, sticky="ew")
            make_entry(f, v1, width=8, bg=BG_INPUT_BLUE
                       ).grid(row=r, column=3, padx=2)
            make_entry(f, v2, width=8, bg=BG_INPUT_BLUE
                       ).grid(row=r, column=4, padx=2)
            e3 = make_entry(f, v3, width=8, bg=BG_INPUT_BLUE)
            e3.grid(row=r, column=5, padx=2)
            e4 = make_entry(f, v4, width=8, bg=BG_INPUT_BLUE)
        ...
        _freundlich_row(2, "PSB's Freundlich Kf",
                        ...
                        "OpenTable9_2")
        # Converted Kf row — UNIT cell black per user feedback.
        _freundlich_row(3, "Converted PSB's Freundlich",
                        ...
                        None,
                        unit_bg=BG_LOCKED, unit_fg="#FFFFFF")

        # ═══ RIGHT — Total Width / fcac / # of cells stack ══════════════
        for i, (lbl, var, unit, helpm) in enumerate([
            ...
            tk.Label(right, text=lbl, font=FONT_LABEL, bg=BG_MAIN
                     ).grid(row=i, column=0, sticky="w", pady=1)
            make_entry(right, var, width=6).grid(row=i, column=1, padx=4)
            tk.Label(right, text=unit, font=FONT_LABEL_SM, bg=BG_MAIN,
                     fg="#C00000"
                     ).grid(row=i, column=2, sticky="w")
            if helpm:
                help_link(right, helpm).grid(row=i, column=3, padx=2)
    ...
    #
    # 3-column layout (per user image):
    #   ┌─────────────┬──────────────────────────┬─sep─┬───────────────────┐
    #   │ 10. FIELD   │ Monitoring Well Name     │  │  │ Monitoring Well   │
    #   │ DATA TO     │   PFOS  None             │  │  │ Location Info:    │
    #   │ CALIBRATE   │ Event PFOS None  (ug/L)  │  │  │ Distance ?        │
    #   │             │ MW-504 2000  __  (ug/L)  │  │  │ For simple ...10  │
    #   │ Sample Year │ ...                      │  │  │ ...               │
    #   │ [2025] ?    │                          │  │  │                   │
    #   │             │                          │  │  │                   │
    #   │ Enter up to │                          │  │  │                   │
    ...
    def _build_s10_field_data(self, parent):
        ...
        outer.pack(fill="x", pady=(4, 4))

        # ═══ FAR LEFT — multi-line section header + Sample Year + notes ═
        ...
        tk.Label(leftcol, text="Sample Year (XXXX)",
                 font=FONT_LABEL_I, bg=BG_MAIN
                 ).pack(anchor="w", pady=(0, 0))
        sy_row = tk.Frame(leftcol, bg=BG_MAIN); sy_row.pack(anchor="w")
        make_entry(sy_row, self.v_sample_yr, width=6
                   ).pack(side="left", padx=(0, 4))
        help_link(sy_row, "OpenTable10_1").pack(side="left")

        # Italic helper notes
        helper_fr = tk.Frame(leftcol, bg=BG_MAIN)
        ...
        for txt in ["Enter up to 7 shallow",
            ...
            tk.Label(helper_fr, text=txt, font=FONT_LABEL_SMI,
                     bg=BG_MAIN, fg=FG_GREY, anchor="e"
                     ).pack(anchor="e")

        # ═══ MIDDLE — MW table ════════════════════════════════════════════
        mid = tk.Frame(outer, bg=BG_MAIN)
        ...
        for i in range(7):
            ...
            tk.Entry(mid, textvariable=self.v_mw_conc[i], width=8,
                     font=FONT_INPUT, bg=BG_INPUT_BLUE, fg="#C00000",
                     relief="solid", bd=1, justify="right"
            ...
            tk.Label(mid, text="(ug/L)", font=FONT_LABEL_SMI,
                     bg=BG_MAIN, fg="#C00000"
                     ).grid(row=3+i, column=3, sticky="w", padx=2)

        # ═══ Visual separator (vertical black line) ═══════════════════════
        sep = tk.Frame(outer, bg="#000000", width=2)
        sep.pack(side="left", fill="y", padx=(16, 16))

        # ═══ RIGHT — Location Info header + Distance + per-row notes ════
        rt = tk.Frame(outer, bg=BG_MAIN)
        rt.pack(side="left", anchor="nw")

        # Top header — "Monitoring Well Location Info:"
        ...
        # Distance column header (right-aligned)
        ...
        # Rows 2-8 — italic notes (left) paired with distance values (right)
        for i in range(7):
