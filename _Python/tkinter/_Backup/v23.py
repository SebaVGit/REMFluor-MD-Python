...
class REMFluorApp(tk.Tk):
    ...
    # SECTION 9 – PSB
    #
    # Compact horizontal layout (per user reference image):
    #
    #   ┌──── Freundlich grid ────┐ │ ┌─PSB Dist─┐  ┌─image─┐
    #   │ Model PSB? Unit PFOS .. │ │ │ PSB      │  │       │
    #   │ Freundlich "a" - val .. │ │ │ Dist.    │  │  PSB  │
    #   │ Freundlich Kf  peach .. │ │ │ From     │  │ block │
    #   │ Converted Kf  BLACK ..  │ │ │ Source   │  │       │
    #   └─────────────────────────┘ │ │ [val](m)?│  └───────┘
    #                               │ └──────────┘
    #          Year PSB Installed [v] ?    Total Width   [v] (m) (Typical 4) ?
    #                                       PSB Loading  [v] (%) ?
    #                                       # of cells   [v] (-)
    #
    #   [Where to Get Freundlich]  [Simple CAC Barrier Longevity Tool]
    # ─────────────────────────────────────────────────────────────────────
    def _build_s9_psb(self, parent):
        ...
        # ═══ TOP — grid + separator + PSB Dist + image ════════════════
        top = tk.Frame(parent, bg=BG_MAIN)
        top.pack(fill="x", pady=(0, 2))

        # ── Freundlich grid ────────────────────────────────────────
        f = tk.Frame(top, bg=BG_MAIN)
        ...
        def _freundlich_row(r, label, unit, v1, v2, v3, v4, helpm,
                            ...
                            value_bg=None, locked=False):
            tk.Label(f, text=label, font=FONT_LABEL, bg=BG_MAIN,
                     anchor="e"
                     ).grid(row=r, column=0, sticky="e", pady=1, padx=(0, 4))
            tk.Label(f, text=unit, font=FONT_LABEL_SM, bg=unit_bg,
                     fg=unit_fg, relief="solid", bd=1, padx=2
                     ).grid(row=r, column=1, padx=2, sticky="ew")
            vbg = value_bg if value_bg else BG_INPUT_BLUE
            make_entry(f, v1, width=8, bg=vbg
                       ).grid(row=r, column=2, padx=2)
            make_entry(f, v2, width=8, bg=vbg
                       ).grid(row=r, column=3, padx=2)
            e3 = make_entry(f, v3, width=8, bg=vbg)
            e3.grid(row=r, column=4, padx=2)
            e4 = make_entry(f, v4, width=8, bg=vbg)
            e4.grid(row=r, column=5, padx=2)
            _mark_detailed(e3, e4)
            if helpm:
                help_link(f, helpm).grid(row=r, column=6, padx=(2, 0))
        ...
        tk.Label(dist_row, text="(m)", font=FONT_LABEL_SM,
                 bg=BG_MAIN, fg="#C00000"
                 ).pack(side="left", padx=(2, 0))
        help_link(dist_row, "OpenTable9_3").pack(side="left", padx=(2, 0))

        # ── Image (Section9_1.png) ──────────────────────────────────
        scale = getattr(self, "_dpi_scale", 1.0)
        self._figures["s9_1"] = _load_figure("Section9_1.png",
                                             target_height=int(70 * scale))
        if self._figures["s9_1"] is not None:
            tk.Label(top, image=self._figures["s9_1"], bg=BG_MAIN, bd=0
                     ).pack(side="left", anchor="n")

        # ═══ MIDDLE — Year PSB (left) + Total Width stack (right) ═══
        mid = tk.Frame(parent, bg=BG_MAIN)
        mid.pack(fill="x", pady=(2, 2))

        # Year PSB Installed sits inline near the PSB-Dist column,
        # closer to the figure (matches Excel layout).
        yr = tk.Frame(mid, bg=BG_MAIN)
        yr.pack(side="left", anchor="nw", padx=(220, 0))
        ...
        rg = tk.Frame(mid, bg=BG_MAIN)
        rg.pack(side="right", anchor="ne", padx=(0, 8))
        for i, (lbl, var, unit, helpm) in enumerate([
            ...
            tk.Label(rg, text=unit, font=FONT_LABEL_SM,
                     bg=BG_MAIN, fg="#C00000"
        ...
        make_btn(bf, "Where to Get Freundlich Parameters",
                 ...
                 font=FONT_BTN, width=38, bg=BTN_FILL,
                 padx=14, pady=8
                 ).pack(side="left", padx=(0, 12))
        make_btn(bf, "Simple CAC Barrier Longevity Tool",
                 "LongevityTool", font=FONT_BTN, width=38, bg=BTN_FILL,
                 padx=14, pady=8
                 ).pack(side="left")
    ...
    def _build_s11_output(self, parent):
        ...
        # while the surrounding panel stays blue.
        make_btn(bar, "Run Model", "RunPythonScript",
                 fg=FG_BTN_NAVY, font=FONT_BTN_LG, padx=12, pady=10,
                 bg=BTN_FILL, width=14).pack(side="left", padx=6)
        make_btn(bar, "Run Model with\nAuto-Calibration",
                 "Show_Visualization",
                 fg=FG_BTN_NAVY, font=FONT_BTN_LG, padx=12, pady=10,
                 bg=BTN_FILL, width=18).pack(side="left", padx=6)
