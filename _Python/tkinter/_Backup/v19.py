...
class REMFluorApp(tk.Tk):
    ...
    def _toggle_widgets(self, widgets, show):
        """Show or hide widgets regardless of geometry manager.

        After ``grid_remove()`` ``winfo_manager()`` returns "" but
        ``grid_info()`` still holds the preserved coordinates — so we
        use ``grid_info()`` as the reliable marker for grid-managed
        widgets, and a cached ``_pack_info`` attribute for pack-managed.
        """
        for w in widgets:
            try:
                # Pack-managed (currently or previously hidden)?
                if hasattr(w, "_pack_info"):
                    if show:
                        w.pack(**w._pack_info)
                    else:
                        w.pack_forget()
                    continue
                # Grid-managed?  grid_info() returns a non-empty dict
                # both when the widget is currently visible AND after a
                # grid_remove() (which preserves the options).
                gi = None
                try:
                    gi = w.grid_info()
                except Exception:
                ...
                if gi:
                    if show:
                        ...
                        w.grid_remove()
                    continue
                # Otherwise check current manager (covers
                # pack-managed widgets we haven't hidden yet).
                mgr = ""
                try:
                    mgr = w.winfo_manager()
                except Exception:
