...
def _launch_exe(exe_path: str, args: list, wait: bool = True):
    ...
    # After every EXE finishes (sync runs), pull the updated cell values
    # back from the .xlsm into the Tk UI so the user sees the result.
...
class REMFluorApp(tk.Tk):
    ...
    def _on_pfaa_change(self, *_):
        """Recompute Section 5 retardation cells using the EXACT VBA
        formula (Sheet3.cls.HandlePFAS_Generic):
            R_high = 1 + (Koc * 0.001 * 1.7 / G22)   ; G22 = trans. porosity
            R_low  = 1 + (Koc * 0.002 * 1.5 / K27)   ; K27 = low-k porosity
        Also writes the molecular diffusion coefficient (PFAA-1 only)."""
        try:
        ...
        for idx, (src_var, ret_t, ret_l) in enumerate([
            (self.v_pfaa1, self.v_ret_trans1, self.v_ret_lowk1),
            (self.v_pfaa2, self.v_ret_trans2, self.v_ret_lowk2),
        ]):
            koc, diff = self.PFAA_KOC.get(src_var.get(), (None, None))
            if koc is None:
                ret_t.set("");  ret_l.set("")
                continue
            r_high = 1 + (koc * 0.001 * 1.7 / G22) if G22 > 0 else 1.0
            r_low  = 1 + (koc * 0.002 * 1.5 / K27) if K27 > 0 else 1.0
            ret_t.set(f"{r_high:.2f}")
            ret_l.set(f"{r_low:.2f}")
            # Molecular diffusion only from PFAA-1 (per VBA HandlePFAS_E38)
    ...
    def _on_lowk_media_change(self, *_):
        ...
        por, tort = self.LOWK_DEFAULTS.get(self.v_lowk_media.get(),
                                           ("", ""))
        self.v_lowk_por.set(str(por))
        self.v_lowk_tort.set(str(tort))
    ...
    def _build_header_strip(self, parent):
        ...
        tk.Radiobutton(ver_box, text="Simple Version",
                       variable=self.v_model_version, value="Simple Version",
                       font=FONT_LABEL, bg=BG_MAIN
                       ).pack(anchor="w")
        tk.Radiobutton(ver_box, text="Detailed Version",
        ...
        tk.Radiobutton(u_box, text="feet",   variable=self.v_units, value="feet",
                       font=FONT_LABEL, bg=BG_MAIN).pack(anchor="w")
        tk.Radiobutton(u_box, text="meters", variable=self.v_units, value="meters",
                       font=FONT_LABEL, bg=BG_MAIN).pack(anchor="w")
    ...
    def _build_s5_transport(self, parent):
        ...
        for i, (lbl, var1, var2, unit, helpm) in enumerate(rows):
            ...
            # PFAA-1 cell (col 1) — BLACK locked: value comes from
            # "Calculate Retardation Factors" button (per Excel).
            make_entry(f, var1, width=8, bg=BG_LOCKED).grid(
                row=2+i, column=1, padx=4)
            # PFAA-2 cell (col 2) — same lock state
            make_entry(f, var2, width=8, bg=BG_LOCKED).grid(
        ...
        cb = tk.Frame(parent, bg=BG_MAIN); cb.pack(anchor="center", pady=(2,2))
