...
def run_script(macro_name, extra_args=None):
    ...
    if macro_name == "Load_Optimal_Data":
        ...
        try:
            ...
            # (3) Bake calibration multipliers into §7 cells and
            #     reset Mid to 1 (matches post-calibration behavior).
            cb = getattr(_app_ref, "_apply_calib_multipliers_to_s7", None)
            if callable(cb):
                try: cb()
                except Exception: pass

            # (4) Recompute retardation from the loaded PFAA species
    ...
    def _build_calibration_panel(self, parent):
        ...
        def _apply_calib_multipliers_to_s7():
            ...
            for label, src_list in ROWS:
                ...
                # Reset mid to 1 so the next calibration / run
                # doesn't compound the multiplier.
                try: mid[idx].set("1")
                except Exception: pass
