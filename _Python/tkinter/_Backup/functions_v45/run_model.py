...
def run(app, parent=None) -> bool:
    ...
    label = tk.Label(root, text="Starting...", font=("Arial", 26, "bold"),
                     bg="#F0F0F0", fg="#222", padx=32, pady=24,
                     justify="center", wraplength=720)
    label.pack(padx=36, pady=(36, 12), fill="x")
    ...
    detail = tk.Label(root,
                      ...
                      justify="center", wraplength=720)
    detail.pack(padx=36, pady=(0, 28))

    # Center & size after layout — bigger minimum so user can read clock easily
    root.update_idletasks()
    w = max(root.winfo_reqwidth() + 48, 780)
    h = max(root.winfo_reqheight() + 36, 380)
    try:
