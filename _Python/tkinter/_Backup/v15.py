...
class REMFluorApp(tk.Tk):
    ...
    def _toggle_widgets(self, widgets, show):
        ...
        After pack_forget() winfo_manager() returns "" — so we can't
        use it to detect a previously-packed widget on the way back.
        Instead, treat any widget with a cached ``_pack_info`` attr
        as pack-managed (currently or formerly).
        """
        for w in widgets:
            try:
                ...
                if mgr == "grid":
                    ...
                    if not show:
                        # First hide — cache info then forget.
                        w._pack_info = w.pack_info()
                        w.pack_forget()
                    # show + already packed → nothing to do.
            except Exception:
                pass
