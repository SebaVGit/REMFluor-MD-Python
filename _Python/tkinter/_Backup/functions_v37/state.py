...
CLEAR_CELLS = [
    ...
    *[f"AB{r}" for r in range(8, 19)],
    ...
    # Section 9 PSB (all fields)  ← previously incomplete
    "V23", "X23", "Z23", "AB23",
    "V24", "X24", "Z24", "AB24",
    "V26",                              # psb_kf_conv  ← was missing
    "Y75",                              # psb_yr       ← was missing
    "X74",                              # psb_dist     ← was missing
    "Y82",                              # psb_width    ← was missing
    "AA82",                             # psb_load     ← was missing
    ...
    *[f"X{r}" for r in range(34, 41)],
    # PFAS names & retardation (Section 5)
    "E38", "G38", "K38", "M38",
...
class AppState:
    ...
    def push(self, app):
        """Write state back to UI widgets."""
        import tkinter as tk
        for addr, var_name in CELL_MAP.items():
            val = self.get(addr)
            if re.search(r'_\d+$', var_name):
