...
class REMFluorApp(tk.Tk):
    ...
    def _build_s7_source(self, parent):
        ...
        # ── Row 0 – Column headers (italic gray per Excel reference) ─
        tk.Label(f, text="PFAA 1", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY
                 ).grid(row=0, column=3, padx=4)
        tk.Label(f, text="PFAA 2", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY
        ...
        # Excel reference cell colors:
        #   * Year column   → BLACK locked (calculated from §2)
        #   * Conc columns  → SKY BLUE (formula / overwriteable)
        for i in range(11):
            r = 2 + i
            make_entry(f, self.v_src_years[i], width=8,
                       bg=BG_LOCKED, justify="right"
                       ).grid(row=r, column=2, padx=2, pady=1)
            make_entry(f, self.v_src_pfaa1[i], width=10, bg=BG_FORMULA
                       ).grid(row=r, column=3, padx=2, pady=1)
            make_entry(f, self.v_src_pfaa2[i], width=10, bg=BG_FORMULA
                       ).grid(row=r, column=4, padx=2, pady=1)
            tk.Label(f, text="(ug/L)", font=FONT_LABEL_SMI,
                     bg=BG_MAIN, fg=FG_GREY
                     ).grid(row=r, column=5, sticky="w", padx=2)
            e_pre1 = make_entry(f, self.v_src_pre1[i], width=10, bg=BG_FORMULA)
            e_pre1.grid(row=r, column=6, padx=2, pady=1)
            e_pre2 = make_entry(f, self.v_src_pre2[i], width=10, bg=BG_FORMULA)
