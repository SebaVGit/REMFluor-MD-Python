...
def run_script(macro_name, extra_args=None):
    ...
    if macro_name == "Load_Optimal_Data":
        ...
        try:
            ...
            # Push Mid → §1-§7 inputs (also guarded by _calib_pushing
            # so the v_darcy trace doesn't fight us).
            cb = getattr(_app_ref, "_push_calib_mids_to_inputs", None)
            if callable(cb):
                try: cb()
                except Exception: pass
            # Recompute retardation from the loaded PFAA species +
            # porosity so v_ret_trans* line up with the restored
            # source-cell snapshot.
            try: _app_ref._on_pfaa_change()
            except Exception: pass
            messagebox.showinfo("Load Optimal Data",
                                f"Restored {n} calibration row(s) from:\n"
                                f"{path}\n\n"
                                "Source cells (§3 v_darcy, §3 porf, "
                                "§5 retardation, §6 alphax) restored.\n"
    ...
    if macro_name == "Save_Optimal_Model":
        ...
        try:
            ...
            with open(path, "w", encoding="utf-8") as fp:
                fp.write("REMFluor-MD Optimal Model snapshot v2\n")
                fp.write(f"Iterations: {_app_ref.v_n_iter.get()}\n")
                # Snapshot the §3 / §5 / §6 source cells too —
                ...
                for i, label in enumerate(_CALIB_PARAMS):
                    ...
                    fp.write(f"  mid={_app_ref.v_calib_mid[i].get()}\n")
...
class REMFluorApp(tk.Tk):
    ...
    def _build_calibration_panel(self, parent):
        ...
        # panel would show the optimum but Ru