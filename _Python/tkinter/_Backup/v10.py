...
class REMFluorApp(tk.Tk):
    ...
    def _build_vars(self):
        ...
        # Section 5 — Detailed-only Transformation Rate + Yield Factor
    ...
    def _build_s5_transport(self, parent):
        ...
        # White input cells with RED text in the precursor columns.
        for r_off, lbl, v3, v4, unit in [
            ...
            tr_lbl.grid(row=r_off, column=0, sticky="e",
                        pady=1, padx=(0, 6))
            # Empty cells in PFAA1/PFAA2 columns to keep grid alignment
        ...
        # "Calculate Retardation Factors" button — centered below the
        # retardation cells, BEFORE the Diffusion Coefficient row.
        cb = tk.Frame(parent, bg=BG_MAIN); cb.pack(anchor="center", pady=(3, 3))
        make_btn(cb, "Calculate Retardation Factors",
                 "CalculrateRetardationFactors",
                 font=FONT_BTN_SM, width=24, bg=BTN_FILL
                 ).pack(side="left", padx=2)
        help_link(cb, "OpenTable5_2").pack(side="left", padx=4)

        # Detailed-only "Modeling Transformation Low-K" button (right side)
        mt_btn = make_btn(cb, "Modeling Transformation Low K",
                          "ModelingTransformationLowK",
                          font=FONT_BTN_SM, width=28, bg=BTN_FILL)
        mt_btn.pack(side="left", padx=(12, 2))
    ...
    def _build_s8_source_rem(self, parent):
        ...
        # ── Blue chevron arrows (per Excel storyboard) ───────────────
        # Two stacked downward arrows pointing at the Section 8 image,
        # rendered as filled polygons on a tk.Canvas. Visible in BOTH
        # Simple and Detailed.
        arrow_fr = tk.Frame(parent, bg=BG_MAIN)
        arrow_fr.pack(fill="x", pady=(4, 0))
        cnv = tk.Canvas(arrow_fr, width=80, height=110,
                        bg=BG_MAIN, highlightthickness=0)
        cnv.pack(side="left", padx=(0, 4))
        # Top arrow (right-pointing)
        cnv.create_polygon(
            5, 10, 35, 10, 35, 0, 60, 22, 35, 44, 35, 34, 5, 34,
            fill="#4F81BD", outline="#385D8A", width=1)
        # Bottom arrow (down-pointing)
        cnv.create_polygon(
            10, 55, 30, 55, 30, 80, 40, 80, 20, 105, 0, 80, 10, 80,
            fill="#4F81BD", outline="#385D8A", width=1)
    ...
    # SECTION 9 – PSB
    #
    # XLSM-authoritative layout (Detailed_2):
    #   Headers row 22:   U=Unit  V=PFAA1  X=PFAA2  Z=Pre1  AB=Pre2
    #   Row 23 — PSB's Freundlich "a"   (V/X/Z/AB values)
    #   Row 24 — PSB's Freundlich Kf    (V/X/Z/AB values)
    #   Row 26 — Converted Freundlich Kf (V/X/Z/AB values)
    #   Row 28 — Year PSB Installed (right side, AA28/AB28)
    # ─────────────────────────────────────────────────────────────────────
    def _build_s9_psb(self, parent):
        ...
        # Helper for one Freundlich-row: 2 always-on cells + 2 detailed
