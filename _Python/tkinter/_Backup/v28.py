...
class REMFluorApp(tk.Tk):
    ...
    def _build_s9_psb(self, parent):
        ...
        # Row 3: S molecular weight (g/mol) — only visible when the
        # Kf unit selected on row 2 contains "mol".  All four data
        # cells are BLACK (calculated/locked) per the storyboard.
        # In Simple mode, only cols 2-3 are shown; in Detailed, all 4.
        mw_lbl  = tk.Label(f, text="S molecular weight (g/mol)",
                           font=FONT_LABEL, bg=BG_MAIN, anchor="e")
        mw_lbl.grid(row=3, column=0, sticky="e", pady=1, padx=(0, 4))
        mw_unit = tk.Label(f, text="(g/mol)", font=FONT_LABEL_SM,
                           bg=BG_LOCKED, fg="#FFFFFF",
                           relief="solid", bd=1, padx=2)
        mw_unit.grid(row=3, column=1, padx=2, sticky="ew")
        mw_e1 = make_entry(f, self.v_psb_mw_1, width=8, bg=BG_LOCKED)
        mw_e1.grid(row=3, column=2, padx=2)
        mw_e2 = make_entry(f, self.v_psb_mw_2, width=8, bg=BG_LOCKED)
        mw_e2.grid(row=3, column=3, padx=2)
        mw_e3 = make_entry(f, self.v_psb_mw_3, width=8, bg=BG_LOCKED)
        mw_e3.grid(row=3, column=4, padx=2)
        mw_e4 = make_entry(f, self.v_psb_mw_4, width=8, bg=BG_LOCKED)
        ...
        self._psb_mw_widgets     = [mw_lbl, mw_unit, mw_e1, mw_e2]
