...
def run(app, parent=None):
    ...
    # Render placeholder rows + blue CAC link UP FRONT so the window
    # sizes itself big enough to show every section before the user
    # clicks Calculate.  _render_results() repopulates these on each
    # Calculate; the blue link stays put.
    def _render_results(longevity=None):
        for w in res_content.winfo_children():
            w.destroy()
        if longevity is None:
            no_sf, sf2x = "— years", "— years"
        else:
            li = int(round(longevity))
            no_sf, sf2x = f"{li} years", f"{li // 2} years"
        for txt, val in (
            ("Estimated longevity (no safety factor):", no_sf),
            ("Estimated longevity (2x safety factor):", sf2x),
        ):
    ...
    _render_results()  # initial paint — placeholders + link visible
    ...
    def _calc():
        ...
        # Render — placeholder rows are reused; only the values change
        _render_results(longevity)
    ...
    # Big initial size — match the Excel "Longevity Calculator" popup so
    # everything (params + results placeholder + CAC link + buttons)
    # is visible without resizing.
    w = max(root.winfo_reqwidth() + 64, 980)
    h = max(root.winfo_reqheight() + 64, 980)
    try:
