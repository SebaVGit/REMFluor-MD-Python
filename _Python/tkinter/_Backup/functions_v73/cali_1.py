...
PARAM_MAP: Dict[str, Callable] = {
    # label                                                  setter(state, value)
    "Source Start Year (nt)":
        lambda s, v: s.set("E18", int(round(v))),
    "Hydraulic Conductivity (k)":
        # k stored implicitly in vd = i*k.  Without separate k storage
        # we override v_darcy directly with the k value; user picks
        # ranges in m/yr.
        lambda s, v: _set_vd(s, v),
    "Hydraulic Gradient (i)":
        # No separate gradient cell — overlaying onto v_darcy too.
        lambda s, v: _set_vd(s, v),
...
def _objective_for_app(app, work_dir, exe_path) -> Callable:
    ...
    def _eval(x):
        ...
        for lbl, val in zip(labels, x):
            setter = PARAM_MAP.get(lbl)
            if setter is not None:
                try: setter(state, float(val))
...
def run(app, parent=None) -> bool:
    ...
    def _on_done():
        ...
        if res is None:
            ...
            return
        # Push best params back to the §calibration Mid cells, then
        # propagate those values into the §1-§7 source inputs so
        # subsequent "Run Optimal Model" actually runs with the
        # optimum (not the user's pre-calibration inputs).
        ...
        try:
            ...
            pass
        # Forward Mid → app source cells via the helper registered
        # in main.py (REVERSE_MAP).  Only writes the rows that have
        # a writable source — sidecar-driven rows (volfrac/difflen/
        # decayf, GW Calc i) are deliberately left alone.
        cb = getattr(app, "_push_calib_mids_to_inputs", None)
        if callable(cb):
            try: cb()
            except Exception: pass

        # Delete sidecar files now that the canonical state lives
        # in the source StringVars (v_darcy, v_porf, etc.).  Without
        # this, any future _refresh_calib_mids — triggered by a
        # GW Calc popup, a Heterogeneity popup, or even just typing
        # into a §3/§5 cell — would re-read the stale sidecar K /
        # volfrac / difflen and clobber the optimum we just pushed
        # into the Mid column.
        try:
            for sidecar in ("gwvelocity_inputs.txt",
                            "heterogeneity_inputs.txt"):
                sp = os.path.join(project, sidecar)
                if os.path.exists(sp):
                    try: os.remove(sp)
                    except Exception: pass
        except Exception:
            pass
        # Recompute retardation from porf + PFAA species so
        # v_ret_trans1..4 line up with the new porf if it changed.
        cb = getattr(app, "_on_pfaa_change", None)
        if callable(cb):
            try: cb()
            except Exception: pass
        # Final refresh — by now sidecars are gone so _src_K /
        # _src_volfrac / _src_difflen all fall back to source vars.
        cb = getattr(app, "_refresh_calib_mids", None)
