...
def build_inp_data(state) -> dict:
    ...
    # Apply §calibration Step 4 multipliers (czero(*,n) rows) to the
    # source columns IF the row is checked AND the Mid value is a
    # non-default multiplier (≠ 1).  This way calibration optima for
    # the multiplier rows actually take effect at Run Optimal Model
    # time — without modifying the §7 source-concentration cells (so
    # the user keeps seeing the baseline values they typed).
    # Multipliers live in state at well-known cell addresses that
    # main.py:Save_Optimal_Model writes, so we read them here.  If
    # they're not in state (no calibration run yet), this is a no-op.
    def _calib_mult(addr, default=1.0):
        try:
            v = state.get(addr)
            if v is None:
                return default
            ...
            return default
    # Cell addresses in main.py XLSM_CELL_MAP (calibration Mid col):
    #   czero(2,n) PFAA-1 multiplier  → AE32  (synthetic addr)
    #   czero(4,n) PFAA-2 multiplier  → AE33
    #   czero(1,n) Pre-1 multiplier   → AE34
    #   czero(3,n) Pre-2 multiplier   → AE35
    # We don't actually have these in CELL_MAP; instead reach into
    # the live app via state.app_ref if available.  Fallback: 1.0.
    app_ref = getattr(state, "app", None) or getattr(state, "_app", None)
    # As a robust fallback, use the import_module trick to grab the
    # global _app_ref from main.py (set on app construction).
    if app_ref is None:
        try:
            import main as _m   # type: ignore
            app_ref = getattr(_m, "_app_ref", None)
        except Exception:
            app_ref = None
    if app_ref is not None:
        try:
            from main import _CALIB_PARAMS  # type: ignore
            chk = getattr(app_ref, "v_calib_chk",  [])
            mid = getattr(app_ref, "v_calib_mid",  [])
            label_to_idx = {lbl: i for i, lbl in enumerate(_CALIB_PARAMS)}

            def _get_mult(label):
                idx = label_to_idx.get(label)
                if idx is None or idx >= len(chk) or idx >= len(mid):
                    return 1.0
            ...
            m_pfaa1 = _get_mult("Multiplier to PFAA-1 Source Concentration in #7 (czero(2,n))")
            m_pfaa2 = _get_mult("Multiplier to PFAA-2 Source Concentration in #7 (czero(4,n))")
            m_pre1  = _get_mult("Multiplier to Precursor-1 Source Concentration in #7 (czero(1,n))")
            m_pre2  = _get_mult("Multiplier to Precursor-2 Source Concentration in #7 (czero(3,n))")

            if m_pfaa1 != 1.0:
                concs2 = [c * m_pfaa1 for c in concs2]
            if m_pfaa2 != 1.0:
                concs4 = [c * m_pfaa2 for c in concs4]
            if m_pre1  != 1.0:
                concs1 = [c * m_pre1  for c in concs1]
            if m_pre2  != 1.0:
                concs3 = [c * m_pre2  for c in concs3]
        except Exception as exc:
            print(f"[generate_input_file] multiplier apply failed: {exc}")
