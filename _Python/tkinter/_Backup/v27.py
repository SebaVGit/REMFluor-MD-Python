...
class REMFluorApp(tk.Tk):
    ...
    def _build_s9_psb(self, parent):
        ...
        # line that links the "PSB Dist From Source" entry chicklet (in
        # `psbd`, immediately to the left of this canvas) with the red
        # SOURCE square on the 3D-box figure.
        scale = getattr(self, "_dpi_scale", 1.0)
        ...
        if img is not None:
            iw = img.width()
            ih = img.height()
            img_canvas = tk.Canvas(rcluster, width=iw, height=ih,
                                   bg=BG_MAIN, highlightthickness=0, bd=0)
            img_canvas.pack(anchor="center", pady=(0, 4))
            img_canvas.create_image(0, 0, anchor="nw", image=img)
            # Source rectangle on the original 1165×365 png is roughly at
            # x≈55, y≈140 (left red square).  Scale into current image
            # size and use that as the curve's terminus.
            sx = max(2, int(iw * 0.06))
            sy = int(ih * 0.42)
            # Smooth curve from canvas bottom-left (visually adjacent to
            # the dist input) up & over to land on the source marker.
            img_canvas.create_line(
                0,                  ih - 2,
                int(iw * 0.03),     int(ih * 0.85),
                int(iw * 0.05),     int(ih * 0.65),
                sx,                 sy,
                smooth=True,
                fill="#7F7F7F", width=2, capstyle="round",
            )

        # Total Width / fcac / # cells stack (centered under image)
        rg = tk.Frame(rcluster, bg=BG_MAIN)
        rg.pack(anchor="center", pady=(0, 4))
        ...
        # Year PSB Installed (centered under Total Width stack)
        yr = tk.Frame(rcluster, bg=BG_MAIN)
        yr.pack(anchor="center", pady=(2, 2))
    ...
    def _build_s11_output(self, parent):
        ...
        actfr.pack(side="left", padx=(20, 0))
        cells = [
        ...
        for r, c, txt, macro, bg, fg in cells:
            make_btn(actfr, txt, macro,
                     fg=fg, font=FONT_BTN, padx=14, pady=8,
                     bg=bg, width=14
