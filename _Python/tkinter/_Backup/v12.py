...
class REMFluorApp(tk.Tk):
    ...
    def _on_model_version_change(self, *_):
        ...
        # Show/hide Detailed-only UI sections
        is_detailed = (self.active_sheet == "Detailed_2")
        for frame in getattr(self, "_detailed_only_frames", []):
            try:
                if is_detailed:
                    frame.grid()
                else:
    ...
    def _on_het_change(self, *_):
        ...
        if preset is None:
            return   # "Enter Your Own Value Below" — leave fields editable
        al, at, av = preset
    ...
    def _build_s7_source(self, parent):
        ...
        hdr_pre1 = tk.Label(f, text="Precursors 1", font=FONT_LABEL_B,
                            bg=BG_MAIN)
        hdr_pre1.grid(row=0, column=6, padx=4)
        hdr_pre2 = tk.Label(f, text="Precursors 2", font=FONT_LABEL_B,
                            bg=BG_MAIN)
        ...
        rot_h = 22 * 11
        rot = tk.Canvas(f, width=18, height=rot_h,
                        bg=BG_MAIN, highlightthickness=0)
        rot.create_text(9, rot_h // 2,
                        text="Go to Section 2 to Change Years",
                        font=FONT_LABEL_I, fill=FG_BTN_NAVY, angle=90)
        rot.grid(row=2, column=1, rowspan=11, sticky="ns", padx=(0, 4))
        ...
        make_btn(opt2, "Upload PFAS\nConcentrations\nfrom PFAS-LEACH",
                 "ImportMassDischarge", font=FONT_BTN_SM, width=24,
        ...
        for i in range(11):
            ...
            make_entry(f, self.v_src_pfaa2[i], width=10
                       ).grid(row=r, column=4, padx=2, pady=1)
    ...
    # SECTION 9 – PSB
    #
    # XLSM-authoritative layout — both Simple and Detailed:
    #
    #   ┌───────────────────────────────────────┬──────────────────────┐
    #   │ Model PSB? │ Unit  │ PFOS │ None │... │ Total Width...   ?   │
    #   │ Freundlich │ -     │ ...  │ ...  │... │ PSB Loading      ?   │
    #   │ Freundlich │peach  │ ...  │ ...  │... │ # of cells       ?   │
    #   │ Conv. Frnd │peach  │ ...  │ ...  │... │                      │
    #   │ Year PSB Installed [val]              │  PSB Dist.  ┌──────┐ │
    #   │ [Where to Get..]  [Simple CAC..]      │  From       │ image│ │
    #   │                                       │  Source [v] │      │ │
    #   │                                       │  (m) ?      └──────┘ │
    #   └───────────────────────────────────────┴──────────────────────┘
    #
    # Simple version: 2 concentration columns (PFOS/None).
    # Detailed:       4 concentration columns (+ Pre1, Pre2).
    # ─────────────────────────────────────────────────────────────────────
    def _build_s9_psb(self, parent):
        ...
        # Outer container — LEFT and RIGHT inner frames sit side-by-side.
        ...
        right.pack(side="left", anchor="nw", padx=(20, 0))

        # ═══ LEFT — Freundlich grid + Year + buttons ════════════════════
        f = tk.Frame(left, bg=BG_MAIN)
        f.pack(fill="x", pady=(0, 4))

        # Row 0 — Model PSB? + species column headers
        ...
        h_pre1 = tk.Label(f, textvariable=self.v_pfaa3, font=FONT_LABEL_B,
                          bg=BG_LOCKED, fg="#FFFFFF",
                          relief="solid", bd=1, width=10)
        h_pre1.grid(row=0, column=5, padx=4, sticky="ew")
        h_pre2 = tk.Label(f, textvariable=self.v_pfaa4, font=FONT_LABEL_B,
                          bg=BG_LOCKED, fg="#FFFFFF",
                          relief="solid", bd=1, width=10)
        h_pre2.grid(row=0, column=6, padx=4, sticky="ew")
        _mark_detailed(h_pre1, h_pre2)

        # Helper for one Freundlich row — Unit cell uses peach (BG_PULLDOWN)
        def _freundlich_row(r, label, unit, v1, v2, v3, v4, helpm,
                            locked=False):
            tk.Label(f, text=label, font=FONT_LABEL, bg=BG_MAIN
                     ).grid(row=r, column=0, sticky="w", pady=1)
            tk.Label(f, text=unit, font=FONT_LABEL_SM, bg=BG_PULLDOWN,
                     relief="solid", bd=1, padx=2
                     ).grid(row=r, column=2, padx=4, sticky="ew")
            bg = BG_LOCKED if locked else BG_INPUT_BLUE
            make_entry(f, v1, width=8, bg=bg).grid(row=r, column=3, padx=2)
            make_entry(f, v2, width=8, bg=bg).grid(row=r, column=4, padx=2)
            e3 = make_entry(f, v3, width=8, bg=bg)
            e3.grid(row=r, column=5, padx=2)
            e4 = make_entry(f, v4, width=8, bg=bg)
            e4.grid(row=r, column=6, padx=2)
            _mark_detailed(e3, e4)
            if helpm:
                help_link(f, helpm).grid(row=r, column=7)

        _freundlich_row(1, 'PSB\'s Freundlich "a"', "-",
                        self.v_psb_a_1, self.v_psb_a_2,
                        self.v_psb_a_3, self.v_psb_a_4,
                        "OpenTable9_1")
        _freundlich_row(2, "PSB's Freundlich Kf",
                        "(mg/kg)(mg/L)^(-a)",
                        self.v_psb_kf_1, self.v_psb_kf_2,
                        self.v_psb_kf_3, self.v_psb_kf_4,
                        "OpenTable9_2")
        _freundlich_row(3, "Converted PSB's Freundlich",
                        "(ug/kg)(ug/L)^(-a)",
                        self.v_psb_kf_conv, self.v_psb_kf_conv2,
                        self.v_psb_kf_conv3, self.v_psb_kf_conv4,
                        None, locked=True)

        # Row 4 — Year PSB Installed
        tk.Label(f, text="Year PSB Installed", font=FONT_LABEL, bg=BG_MAIN
                 ).grid(row=4, column=0, sticky="w", pady=(6, 1))
        make_entry(f, self.v_psb_yr, width=8
                   ).grid(row=4, column=3, padx=2, pady=(6, 1), sticky="w")
        help_link(f, "OpenTable9_3").grid(row=4, column=7, pady=(6, 1))

        # Buttons row — bigger per Excel
        bf = tk.Frame(left, bg=BG_MAIN)
        bf.pack(fill="x", pady=(8, 2))
        make_btn(bf, "Where to Get Freundlich Parameters",
                 "OpenAppendix_9_1_Relative", quarto=True,
                 font=FONT_BTN_SM, width=36, bg=BTN_FILL,
                 padx=8, pady=4
                 ).pack(side="left", padx=(0, 6))
        make_btn(bf, "Simple CAC Barrier Longevity Tool",
                 "LongevityTool", font=FONT_BTN_SM, width=36, bg=BTN_FILL,
                 padx=8, pady=4
                 ).pack(side="left")

        # ═══ RIGHT — Total Width stack (top), then PSB Dist + image ═════
        rg_top = tk.Frame(right, bg=BG_MAIN)
        rg_top.pack(anchor="nw")
        for i, (lbl, var, unit, helpm) in enumerate([
            ...
            tk.Label(rg_top, text=lbl, font=FONT_LABEL, bg=BG_MAIN
                     ).grid(row=i, column=0, sticky="w", pady=1)
            make_entry(rg_top, var, width=6).grid(row=i, column=1, padx=4)
            tk.Label(rg_top, text=unit, font=FONT_LABEL_SM, bg=BG_MAIN,
                     fg="#C00000"
                     ).grid(row=i, column=2, sticky="w")
            if helpm:
                help_link(rg_top, helpm).grid(row=i, column=3, padx=2)

        # Below the stack: PSB Dist. From Source (multi-line) + image
        rg_bot = tk.Frame(right, bg=BG_MAIN)
        rg_bot.pack(anchor="nw", pady=(10, 0))

        # Multi-line label on the LEFT, image on the RIGHT
        psb_dist_fr = tk.Frame(rg_bot, bg=BG_MAIN)
        psb_dist_fr.pack(side="left", anchor="n", padx=(0, 6))
        tk.Label(psb_dist_fr, text="PSB", font=FONT_LABEL, bg=BG_MAIN,
                 fg="#C00000"
                 ).pack(anchor="w")
        tk.Label(psb_dist_fr, text="Dist.", font=FONT_LABEL, bg=BG_MAIN
                 ).pack(anchor="w")
        tk.Label(psb_dist_fr, text="From", font=FONT_LABEL, bg=BG_MAIN
                 ).pack(anchor="w")
        tk.Label(psb_dist_fr, text="Source", font=FONT_LABEL, bg=BG_MAIN
                 ).pack(anchor="w")
        dist_row = tk.Frame(psb_dist_fr, bg=BG_MAIN)
        dist_row.pack(anchor="w", pady=(2, 0))
        ...
        # Image on the right of PSB Dist
        scale = getattr(self, "_dpi_scale", 1.0)
        self._figures["s9_1"] = _load_figure("Section9_1.png",
                                             target_height=int(110 * scale))
        if self._figures["s9_1"] is not None:
            tk.Label(rg_bot, image=self._figures["s9_1"], bg=BG_MAIN, bd=0
    ...
    #
    # XLSM-authoritative layout (Simple):
    #
    #   ┌─────────────────────┬─────────────────────────────────────────┐
    #   │ Sample Year (XXXX)  │ Monitoring Well Name | Conc. Data | Dist│
    #   │ [2025] ?            │ Event | PFOS | None  | (ug/L)     | (m) │
    #   │                     │ MW-504  2000  ____   (ug/L)         10  │
    #   │ Enter up to 7       │ FS-MW504 1950 ____   (ug/L)         50  │
    #   │ shallow centerline  │ ...                                     │
    #   │ wells for key       │                                         │
    #   │ monitoring event    │ + right-side italic "Monitoring Well   │
    #   │                     │   Location Info: For simple model..."  │
    #   └─────────────────────┴─────────────────────────────────────────┘
    #
    # Cell colors:
    #   MW name     — black bg, white text (locked, readonly)
    #   Conc value  — white bg, RED text
    #   Distance    — white bg, RED text
    #   "(ug/L)"    — RED italic gray-bg label
    # ─────────────────────────────────────────────────────────────────────
    def _build_s10_field_data(self, parent):
        ...
        # ═══ LEFT — Sample Year + helper italic notes ═══════════════════
        leftcol = tk.Frame(outer, bg=BG_MAIN)
        leftcol.pack(side="left", anchor="nw", padx=(0, 12))

        sy = tk.Frame(leftcol, bg=BG_MAIN)
        sy.pack(anchor="w", pady=(2, 0))
        ...
        # Italic helper notes (XLSM T38..T40)
        for txt in ["Enter up to 7 shallow",
                    "centerline wells",
                    "for key monitoring event"]:
            tk.Label(leftcol, text=txt, font=FONT_LABEL_SMI,
                     bg=BG_MAIN, fg=FG_GREY, anchor="w"
                     ).pack(anchor="w", pady=(8 if txt.startswith("Enter") else 0, 0))

        # ═══ RIGHT — MW table ════════════════════════════════════════════
        f = tk.Frame(outer, bg=BG_MAIN)
        f.pack(side="left", anchor="nw")

        # Row 0 — group titles (italic bold gray)
        tk.Label(f, text="Monitoring", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY).grid(row=0, column=0, padx=4)
        tk.Label(f, text="Monitoring Well", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY).grid(row=0, column=1, columnspan=2, padx=4)
        tk.Label(f, text="Distance", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY).grid(row=0, column=3, padx=4)
        tk.Label(f, text="Monitoring Well Location Info:",
                 font=FONT_LABEL_BI, bg=BG_MAIN, fg=FG_GREY
                 ).grid(row=0, column=4, padx=(12, 4), sticky="w")

        # Row 1 — sub-titles
        tk.Label(f, text="Well Name", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY).grid(row=1, column=0, padx=4)
        tk.Label(f, text="Concentration Data (ug/L)",
                 font=FONT_LABEL_BI, bg=BG_MAIN, fg=FG_GREY
                 ).grid(row=1, column=1, columnspan=2, padx=4)
        tk.Label(f, text="from", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY).grid(row=1, column=3, padx=4)

        # Row 2 — column headers + help links
        tk.Label(f, text="Event", font=FONT_LABEL_B,
                 bg=BG_MAIN).grid(row=2, column=0, padx=4)
        ...
        tk.Label(f, text="Source (m)", font=FONT_LABEL_BI,
                 bg=BG_MAIN, fg=FG_GREY).grid(row=2, column=3, padx=4)
        help_link(f, "OpenTable10_2").grid(row=2, column=0, sticky="ne",
                                            padx=(40, 0))
        help_link(f, "OpenTable10_3").grid(row=2, column=2, sticky="ne",
                                            padx=(60, 0))
        help_link(f, "OpenTable10_4").grid(row=2, column=3, sticky="ne",
                                            padx=(40, 0))
        ...
        # MW name = locked black; conc + distance = white with red text.
        for i in range(7):
            tk.Entry(f, textvariable=self.v_mw_names[i], width=10,
                     font=FONT_INPUT, bg=BG_LOCKED, fg="#FFFFFF",
                     relief="solid", bd=1, justify="left",
                     state="readonly", readonlybackground=BG_LOCKED
                     ).grid(row=3+i, column=0, padx=2, pady=1)
            tk.Entry(f, textvariable=self.v_mw_conc[i], width=8,
                     font=FONT_INPUT, bg="#FFFFFF", fg="#C00000",
                     relief="solid", bd=1, justify="right"
                     ).grid(row=3+i, column=1, padx=2, pady=1)
            tk.Entry(f, textvariable=self.v_mw_conc2[i], width=8,
                     font=FONT_INPUT, bg="#FFFFFF", fg="#C00000",
                     relief="solid", bd=1, justify="right"
                     ).grid(row=3+i, column=2, padx=2, pady=1)
            tk.Entry(f, textvariable=self.v_mw_dist[i], width=6,
                     font=FONT_INPUT, bg="#FFFFFF", fg="#C00000",
                     relief="solid", bd=1, justify="right"
                     ).grid(row=3+i, column=3, padx=2, pady=1)
            tk.Label(f, text=descr[i], font=FONT_LABEL_SMI,
                     bg=BG_MAIN, fg=FG_GREY, anchor="w"
                     ).grid(row=3+i, column=4, sticky="w", padx=(12, 4))

    # SECTION 11
    def _build_s11_output(self, parent):
        # Header row: section title on the left, "Falta et al. (2025)"
        # caption on the right (matches Excel storyboard image 3).
        hdr_row = tk.Frame(parent, bg=BG_MAIN)
        hdr_row.pack(fill="x", anchor="w", pady=(4, 1))
        section_header(hdr_row, "11",
                       "MODEL OUTPUT AND NUMERICAL PARAMETERS"
                       ).pack(side="left", anchor="w")
        tk.Label(hdr_row, text="Falta et al. (2025)",
    ...
    def _build_calibration_panel(self, parent):
        ...
        for i, w in enumerate(defaults):
            # Name + distance linked to Section 10 monitoring well vars
            tk.Entry(s3g, textvariable=self.v_mw_names[i], width=10,
                     font=FONT_INPUT, bg=BG_LOCKED, fg=FG_LOCKED,
                     relief="solid", bd=1, justify="left",
                     state="readonly").grid(row=i+1, column=0, sticky="w", padx=1)
            tk.Entry(s3g, textvariable=self.v_mw_dist[i], width=8,
                     font=FONT_INPUT, bg=BG_LOCKED, fg=FG_LOCKED,
                     relief="solid", bd=1, justify="right",
                     state="readonly").grid(row=i+1, column=1, sticky="w", padx=1)
