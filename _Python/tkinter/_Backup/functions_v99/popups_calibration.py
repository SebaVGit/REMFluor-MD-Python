...
def _import_xlsx_into_app(app, xlsx_path):
    ...
    try:
        ...
        # v97: shared rejection list — header text variants we must
        # never accept as a real well name.  Used by both the Location
        # sheet loop AND the Data sheet fallback.  Lower-case, stripped.
        BAD_NAMES = {
        ...
        if loc_sheet is not None:
            # v97: more permissive header detection — any header text
            # containing "name" qualifies as a name column ("Location
            # Name", "Well Name", "Monitoring Well Name", "Site Name",
            # etc).  Same for distance: any "distance" / "from source".
            header_row = None
            ...
            for r in range(header_row + 1, header_row + 100):
                ...
                # v97: reject ANY string that's just a header-like
                # placeholder, not just the few short ones we used to
                # reject.  This catches "Monitoring Well Name" etc.
                ...
                if idx >= 6:
                    break
            _log(f"Wells imported from Location sheet (in order): "
                 f"{seen_wells}")
        ...
        # v95: detect Data sheet header row up-front so the fallback
        # doesn't hardcode row 2 (which would treat "Monitoring Well
        # Name" header as a fake well name when data starts on row 3).
        data_header_row = 1
        ...
        if loc_sheet is None and data_sheet is not None:
            ...
            wells_in_data = []
            # v97: BAD_NAMES is now shared with the Location loop above.
            for r in range(data_header_row + 1,
                ...
                if len(wells_in_data) >= 7:
                    break
            _log(f"Wells extracted from Data sheet (in order): "
                 f"{wells_in_data}")
            mw_names = getattr(app, "v_mw_names", []) or []
