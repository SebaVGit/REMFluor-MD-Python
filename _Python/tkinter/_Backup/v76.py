...
def run_script(macro_name, extra_args=None):
    ...
    if macro_name == "Load_Optimal_Data":
        ...
        try:
            ...
            # so the v_darcy trace doesn't fight us).  v_darcy is
            # set to K × i jointly inside this helper.
            cb = getattr(_app_ref, "_push_calib_mids_to_inputs", None)
            if callable(cb):
                try: cb()
                except Exception: pass
            # Re-create gwvelocity_inputs.txt with the loaded K + i so
            # the §3 GW Velocity Calculator pre-fills with these
            # values next time the user opens it (and so subsequent
            # _refresh_calib_mids reads the right values).
            try:
...
class REMFluorApp(tk.Tk):
    ...
    def _build_vars(self):
        ...
        self.v_src_pfaa1 = [tk.StringVar(value="1,600.00") for _ in range(11)]
        ...
        self.v_src_pre1  = [tk.StringVar(value="1,600.00") for _ in range(11)]
