...
# v82: openpyxl, win32com, xlwings imports removed.  The dashboard no
# longer reads cell values from the .xlsm Storyboard workbook —
# run_model.py writes a `dashboard_state.json` snapshot of every state
# value the dashboard needs before launching this subprocess.  This
# eliminates the COM/openpyxl dependency entirely and lets the Phase 3
# .exe build run with no Excel installation present.
#
# The optional calibration .xlsx (Model Data sheet, observed PFAS
# time-series) is still read via openpyxl below (deferred import) —
# that's user-supplied observation data, not Storyboard state.

# v82: state accessors (replaced openpyxl ws lookups)
# `_STATE` is loaded from dashboard_state.json by main(); these helpers
# retain the legacy (ws, ref) signatures so existing call sites work.
_STATE = {}


def _to_number(v):
    """Coerce a state value to a number where possible.  State stores
    everything as strings (StringVar.get()), so V47="100" round-trips
    to int(100)."""
    if v is None:
        return None
...
def read_model_data_sheet(excel_app, calibration_file_path):
    ...
    v82: openpyxl-based — no Excel COM dependency.  excel_app is ignored
    (kept in signature for backward compat with the legacy call site).
    Returns columns [date, analyte, concentration, unit, well] or None.
    """
...
def main(workbook_path=None, sheet_name=None):
    # v82: locate working directory
    # The legacy entry point received the .xlsm path as argv[1] and
    # cwd'd into its parent so all output files (.out, input.inp,
    # dashboard_state.json) resolve by relative name.  We keep that
    # contract so run_model.py's argv stays unchanged.
    ...
    # Load JSON state dump (replaces openpyxl/win32com cell reads).
    # run_model._dump_dashboard_state writes this file before launching.
    global _STATE
    _STATE = {}
    ...
    try:
        ...
        print(f"Warning: could not load dashboard_state.json ({e}); "
              f"plots may be missing context cells")
    ...
    # v82: calibration .xlsx (optional, openpyxl only)
    # The Excel COM Dispatch + Workbooks.Open block was removed.
    # MW_names now come from the §10 monitoring well column (U34:U40)
    # which lives in dashboard_state.json for both Simple and Detailed.
    # The user's separate calibration .xlsx (observed time-series) is
    # still read by read_model_data_sheet() — but with openpyxl, not COM.
    excel = None  # legacy variable, retained for read_model_data_sheet sig
    ...
    # v82: defensive defaults for scalars used in arithmetic.  If the
    # state dump didn't carry a value (cell empty / form not filled),
    # we'd otherwise hit `None - df['Z']` → TypeError downstream.
    def _scalar(addr, default, name):
        v = get_cell_value(ws, addr)
        if v is None or not isinstance(v, (int, float)):
            print(f"[dashboard] {name} ({addr}) missing/non-numeric, "
                  f"using default {default}")
            return default
        return v

    StartY = _scalar("E18", 2025, "Start Year")
    Total_Depth = _scalar("E13", 0.0, "Total Depth")
    ...
    # v83: pad MW_names so an empty/short §10 monitoring-well column
    # (user hasn't filled in well names yet) doesn't crash with
    # "IndexError: list index out of range" inside the loop below.
    # Pre-v82 the .xlsm had cached defaults that masked this.
    ...
    for idx, fname in enumerate(obs_well_files):
        if idx == 0:
            ...
            df['well_file'] = MW_names[idx]
            dfs.append(df)
    df_obs_MW = pd.concat(dfs, ignore_index=True) if dfs else pd.DataFrame() # monitoring wells
    ...
    # v82: defensive defaults — same _scalar helper as above.
    Source_Width = _scalar("E15", 0.0, "Source Width")
    Source_Thick = _scalar("E16", 0.0, "Source Thickness")
    Velocity     = _scalar("E22", 0.0, "Velocity")
    EndYear      = _scalar("E19", StartY + 100, "End Year")
    ...
    # Filter Nones from SL_Time so np.diff doesn't blow up on bad rows
