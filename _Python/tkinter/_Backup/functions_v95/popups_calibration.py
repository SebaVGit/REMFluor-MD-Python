...
def _import_xlsx_into_app(app, xlsx_path):
    ...
    try:
        ...
        if loc_sheet is None and data_sheet is not None:
            ...
            wells_in_data = []
            # v95: start AFTER the detected header row so we don't
            # accidentally pick up the header text "Monitoring Well
            # Name" as a fake well.  Also expanded the rejection list.
            BAD_NAMES = {
                "well", "location", "name", "site",
                "monitoring well", "monitoring well name",
                "well name", "well id", "site name", "id",
                "location name",
            }
        ...
        if data_sheet is not None:
            latest = {}
            # v95: reuse the already-detected header row so the obs
            # parser starts at the right offset.
