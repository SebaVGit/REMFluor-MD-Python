...
def _objective_for_app(app, work_dir, exe_path) -> Callable:
    ...
    # v91: replace -1e9 sentinel with a finite penalty.  The old
    # value showed up as 1,000,000,000 in run_history.csv whenever
    # the solver / template / observation comparison failed, which
    # made the CSV unreadable.  -100 still strongly penalises a
    # failed eval (RMSLE of 100 in log space is huge — typical good
    # fits are 0.1–1.0) but stays human-readable.  Every failure
    # also gets logged to cali_debug.log so we can diagnose later.
    FAIL_PENALTY = -100.0
    debug_log_path = os.path.join(work_dir, "cali_debug.log")

    def _log_fail(reason):
        try:
            with open(debug_log_path, "a", encoding="utf-8") as fp:
                fp.write(f"FAIL: {reason}\n")
        except Exception:
            pass

    # Truncate the debug log at the start of each calibration run
    try:
        with open(debug_log_path, "w", encoding="utf-8") as fp:
            fp.write("REMFluor-MD calibration debug log\n")
            fp.write(f"sample_year={sample_year}, start_year={start_year}, "
                     f"target_t={target_t}\n")
            fp.write(f"obs_pfaa1={obs_pfaa1}\n")
            fp.write(f"obs_pfaa2={obs_pfaa2}\n\n")
    except Exception:
        pass

    # Pre-flight: are there any non-NaN observations at all?  If not,
    # every eval will fall through to the "no obs" sentinel.  Better
    # to refuse to start than fill run_history.csv with FAIL_PENALTY.
    import math as _math
    has_any_obs = any(not _math.isnan(v) for v in obs_pfaa1 + obs_pfaa2)
    if not has_any_obs:
        _log_fail("Pre-flight: NO observed concentrations in §10 "
                  "(v_mw_conc and v_mw_conc2 both empty/NaN).")

    # Cap on what we'll allow Source Start Year to take.  If the
    # user is calibrating "Source Start Year (nt)" and DDS proposes
    # a year > min(U8:U18) of the source-loading times, the
    # generate_input_file step computes negative source times
    # (U8 - startT < 0) which the Fortran solver can't handle.
    # We clamp the proposal to <= min(U8:U18) below.
    try:
        u_times = []
        for k in range(11):
            v = baseline.get(f"U{8 + k}")
            if v is None: continue
            try: u_times.append(float(str(v).replace(",", "").strip()))
            except (ValueError, TypeError): continue
        u_min_year = min(u_times) if u_times else None
    except Exception:
        u_min_year = None
    ...
    def _eval(x):
        ...
        for lbl, val in zip(labels, x):
            ...
            # v91: clamp Source Start Year to <= min(U8:U18) so the
            # input.inp source times stay non-negative.
            if lbl == "Source Start Year (nt)" and u_min_year is not None:
                val = min(float(val), u_min_year)
            if callable(entry):
        ...
        if not os.path.exists(template_path):
            bd = getattr(state, "bundle_dir", "") or proj
            alt = os.path.join(bd, "template.inp")
            if os.path.exists(alt):
                template_path = alt
        out_path      = os.path.join(proj, "input.inp")
        if not os.path.exists(template_path):
            _log_fail(f"template.inp not found at {template_path}")
            return FAIL_PENALTY
        try:
            ...
            _log_fail(f"template fill failed: {exc}")
            return FAIL_PENALTY

        # Run solver
        try:
            cmd = '"{}" < "input.inp" > "output.out"'.format(exe_path)
            cp = subprocess.run(cmd, shell=True, cwd=proj, timeout=600,
                                capture_output=True)
            if cp.returncode != 0:
                _log_fail(f"solver exited with code {cp.returncode}; "
                          f"stderr_tail={(cp.stderr or b'').decode(errors='replace')[-200:]}")
                # Don't fail — solver sometimes returns non-zero
                # but still writes valid obs_well files.  Continue.
        except Exception as exc:
            _log_fail(f"solver subprocess failed: {exc}")
            return FAIL_PENALTY

        # Compare obs_well*.out @ target_t vs typed observations
        wells = _read_obs_wells(proj)
        if not wells:
            _log_fail("no obs_well*.out files produced by solver")
            return FAIL_PENALTY
        ...
        for i, name in enumerate(well_names[:7]):
            ...
            # row index 2 = C2well (PFAA-1), 4 = C4well (PFAA-2)
            sim_c2 = best_row[2] if len(best_row) > 2 else 0.0
            sim_c4 = best_row[4] if len(best_row) > 4 else 0.0
        ...
        if not all_obs:
            _log_fail("no observations matched any well at target_t — "
...
def run(app, parent=None) -> bool:
    ...
    # v91: pre-flight — without §10 observations the optimizer has
    # nothing to compute RMSLE against, every iteration would return
    # FAIL_PENALTY, run_history.csv would be useless.  Refuse to start.
    obs1 = []
    obs2 = []
    for v in getattr(app, "v_mw_conc", [])[:7]:
        try:
            obs1.append(float(str(v.get()).replace(",", "").strip()))
        except (ValueError, TypeError, AttributeError):
            pass
    for v in getattr(app, "v_mw_conc2", [])[:7]:
        try:
            obs2.append(float(str(v.get()).replace(",", "").strip()))
        except (ValueError, TypeError, AttributeError):
            pass
    if not (obs1 or obs2):
        messagebox.showerror(
            "Run Machine Based Calibration",
            "No observed concentrations in §10.\n\n"
            "Calibration needs at least one row of monitoring well\n"
            "concentrations (v_mw_conc / v_mw_conc2) to compute RMSLE.\n"
            "Fill in the §10 grid (or click Paste Example) and try again.")
        return False
    ...
    def _on_done():
        ...
        try:
            ...
            payload = {
                ...
                "labels":     list(labels),
                "best_x":     [float(v) for v in res["best_x"]],
                "n_evals":    int(len(res["x_history"])),
            }
