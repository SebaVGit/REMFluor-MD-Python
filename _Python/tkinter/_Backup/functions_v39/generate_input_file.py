...
def build_inp_data(state) -> dict:
    ...
    precursor_flag = (version_flag == 2 and k38 and
                      str(k38).strip() not in ('', 'None', 'none', 'PFAA 1-able'))
    ...
    # grid
    x1 = c(state.get("AE25")); x1 = x1 or 0
    x2_offset = c(state.get("AH27")); x2_offset = x2_offset or 0
    x2 = x1 + x2_offset
    ...
    if not PSB_flag:
        ...
        if _aa82 is not None and str(_aa82).strip() not in ("", "None"):
            ...
            PSBloading = _safe_float(state.get("AH28", 0))
        tplume1_raw = state.get("AB28") or state.get("AB26")
