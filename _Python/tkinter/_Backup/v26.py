...
class REMFluorApp(tk.Tk):
    ...
    def _build_s9_psb(self, parent):
        ...
        # ═══ TOP — grid + sep + PSB Dist + (image+stack centered) ═══
        top = tk.Frame(parent, bg=BG_MAIN)
        top.pack(fill="x", pady=(0, 2))

        # ── LEFT: Freundlich grid ─────────────────────────────────
        ...
        # ── Connector line — 2 px black, spans section width ──────
        connector = tk.Frame(parent, bg="#000000", height=2)
        connector.pack(fill="x", pady=(2, 0))

        # ═══ BOTTOM — buttons (taller, narrower; closer to top) ════
        bf = tk.Frame(parent, bg=BG_MAIN)
