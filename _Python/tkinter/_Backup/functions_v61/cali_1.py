...
def run(app, parent=None) -> bool:
    ...
    title = tk.Label(root, text="Calibration in Progress",
                     font=("Arial", 22, "bold"),
                     bg="#F0F0F0", fg="#222",
                     padx=24, pady=18, justify="center")
    title.pack(padx=36, pady=(36, 8))

    label = tk.Label(root, text="Starting...",
                     font=("Arial", 26, "bold"),
                     bg="#F0F0F0", fg="#222",
                     padx=24, pady=10, justify="center")
    label.pack(padx=36, pady=(0, 10))

    detail = tk.Label(root, text="",
                      font=("Arial", 14),
                      bg="#F0F0F0", fg="#444",
                      justify="center")
    detail.pack(padx=36, pady=(0, 24))

    stop_flag = {"stop": False}
    btn_row = tk.Frame(root, bg="#F0F0F0"); btn_row.pack(pady=(0, 28))
    def _cancel():
        stop_flag["stop"] = True
        label.config(text="Cancelling...")
    tk.Button(btn_row, text="Cancel", width=14,
              font=("Arial", 13),
              command=_cancel).pack(side="left", padx=12)
    ...
    def _on_done():
        ...
        for child in btn_row.winfo_children():
            child.destroy()
        tk.Button(btn_row, text="Close", width=14, font=("Arial", 13),
                  command=root.destroy).pack(side="left", padx=12)
