...
| **Log-normal** | For parameters that naturally vary over orders of magnitude — hydraulic conductivity is the classic example.  ln(parameter value) is sampled from a normal distribution, and the Step 4 columns hold that distribution's own parameters: the **Lowest column = mu, the mean of ln(parameter value)**, and the **Highest column = sigma, the standard deviation of ln(parameter value)** — the column headers relabel themselves to show this.  The Mid-Range Value (the model's current value) is displayed for reference.  Practical notes: to center the distribution on a known value, enter mu = ln(that value), e.g. mu = ln(5566) = 8.62 for K of 5,566 m/yr; if a site report gives the *variance* of ln K, enter its square root as sigma.  Enter values in the current unit system (feet or meters).  Sigma must be greater than zero, and the choice applies to every checked row. |
